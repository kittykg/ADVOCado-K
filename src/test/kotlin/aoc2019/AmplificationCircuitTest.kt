...
class AmplificationCircuitTest : FunSpec({
    test("example 1.1") {
        ...
        TrialMachine.findMaxThruster(input) shouldBe 43210.toBigInteger()
    }

    test("example 1.2") {
        val input = "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0"
        TrialMachine.findMaxThruster(input) shouldBe 54321.toBigInteger()
    }

    test("example 2.1") {
        val input = "3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5"
        TrialMachine.loopMaxThruster(input) shouldBe 139629729.toBigInteger()
    }
    ...
    test("example 2.2") {
        ...
        TrialMachine.loopMaxThruster(input) shouldBe 18216.toBigInteger()
    }
})
